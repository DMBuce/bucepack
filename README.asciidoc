...
data/minecraft/tags/enchantment/exclusive_set/boots.json
data/minecraft/tags/enchantment/exclusive_set/bow.json
...
and can only pick up plants and fungi.
...
Lets you use ender eyes to find
ruined nether portals in the Nether
and end cities in The End.
...
These packs add or modify recipes and/or enchantments in the vanilla game.
They shouldn't add any lag to your game.
...
saddles, nametags, horse armor, banner patterns, and sherds.
...
* 6 diamond → diamond horse armor
* 6 gold ingot → golden horse armor
* 6 iron ingot → iron horse armor
* 6 leather → leather horse armor
* paper + 1 breeze rod → guster banner pattern
* paper + 1 piglin head → piglin banner pattern
* paper + 1 wind charge → flow banner pattern
* string + 1 paper → name tag
* brick + 1 pottery sherd → 2 pottery sherd

====

In addition, the leather horse armor recipe is changed to match wolf armor
...
data/minecraft/recipe/pale_oak_fence_gate.json
data/minecraft/recipe/pale_oak_fence.json
data/minecraft/recipe/pale_oak_pressure_plate.json
data/minecraft/recipe/pale_oak_sign.json
data/minecraft/recipe/pale_oak_stairs.json
data/minecraft/recipe/pale_oak_trapdoor.json
data/minecraft/recipe/pale_oak_wood.json
...
data/minecraft/recipe/resin_brick_stairs.json
...
data/minecraft/recipe/stripped_pale_oak_wood.json
...
Adds the following custom enchantments that can be obtained from the
enchanting table, chest loot, and mob equipment.

* **Reach** (Trident):
  Increases mob interaction range by 1 block when held in the main hand.
  Increases block interaction range by 2 blocks when held in the main or off hand.
* **Speed I-II** (Leather, Gold, or Chainmail Boots):
  Increases speed by 20% per level.
  Mutually exclusive with Soul Speed.
* **Jump Boost I-II** (Leggings):
  Increases jump strength by 2m/s per level.
* **Darkvision** (Leather, Gold, Chainmail, or Turtle Helmet):
  Gives night vision at
  https://minecraft.wiki/w/Light#Internal_light_level[internal light level]
  3 or less.
...
data/minecraft/loot_table/blocks/pale_hanging_moss.json
...
Saddles, nametags, horse armor, and banner patterns are craftable
