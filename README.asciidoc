...
These data packs only add recipes, change loot tables, or adjust tags.
They don't run any commands and shouldn't lag your game.
...
These data packs keep NBT access to a minimum and only run commands when an
event is triggered in an advancement.
They don't run any ticking commands and shouldn't cause very much, if any,
lag.
...
These data packs haven't been optimized very well yet.
They access NBT data a lot and run commands every tick.
